...
if sys.argv[-1] == 'dry-run':
    ...
    if branch != "master":
        sys.exit("dry-run from a branch other than master is disallowed.")
    os.system("pip uninstall -y hcli_hst")
    os.system("rm -rf requirements.txt")
    write_requirements()
    os.system("rm -rf hcli_hst.egg-info")
...
if sys.argv[-1] == 'publish':
    ...
    os.system("pip install -e .")
    os.system("git tag -a %s -m 'version %s'" % ("hcli_hst-" + version, "hcli_hst-" + version))
    os.system("git push")
